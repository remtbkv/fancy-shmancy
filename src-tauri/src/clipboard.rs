...
fn paste_direct(
    ...
    // Upstream now borrows Enigo from app state for the duration of the type;
    // which of the two typing styles runs inside it is this fork's call.
    with_enigo(app_handle, |enigo| match style {
    ...
    match paste_method {
        ...
        PasteMethod::CtrlV | PasteMethod::CtrlShiftV | PasteMethod::ShiftInsert => {
            ...
            if settings.reliable_paste {
                // Upstream borrows Enigo from app state around the paste; the
                // submit decision stays this fork's (a re-pasted historical
                // transcript must not press Enter).
