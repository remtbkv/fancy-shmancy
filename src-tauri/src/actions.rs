        ...
        match rm.try_start_recording(&binding_id, vad_policy) {
            Ok(readiness) => {
                ...
                std::thread::spawn(move || {
                    ...
                    // Readiness alone isn't enough here: this fork can still drop
                    // a recording that turned out to be an editing chord, and the
                    // start chime must not play for one of those.
        ...
        if recording_error.is_none() {
            ...
            if quiet_ms == 0 {
                ...
                std::thread::spawn(move || {
                    ...
                    set_tray_state(&app_clone, TrayIconState::Recording);
...
impl ShortcutAction for CancelAction {
    fn start(&self, app: &AppHandle, _binding_id: &str, _shortcut_str: &str) {
        utils::cancel_current_operation_from(app, "cancel shortcut (escape)");
    }
...
impl ShortcutAction for PasteLastTranscriptAction {
    fn start(&self, app: &AppHandle, binding_id: &str, shortcut_str: &str) {
        ...
        if let Err(e) = app.run_on_main_thread(move || {
            ...
            let style = crate::paste_target::typing_style(&get_settings(&app_clone));
